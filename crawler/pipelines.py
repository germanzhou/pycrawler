...
_locator = QQwry()
_locator.load_file('qqwry.dat')
logging.getLogger(__name__).info('Loaded qqwry files.')

class DnsRecordPipeline(object):
	INTERESTED_TYPE_STR = '/dns'
	locator = QQwry()
	ignored_ip = ["46.82.174.68","93.46.8.89","78.16.49.15","59.24.3.173",
				"8.7.198.45","243.185.187.39","93.46.8.90","59.24.3.174"]

	def process_item(self, item, spider):
		if self.INTERESTED_TYPE_STR in item['raw_rsp'].meta[spider.TASK_TYPE]:
			result = []
			for ip in item['raw_rsp'].meta[spider.DOMAIN_IP_LIST]:
				ipstr = str(ip)
				if not ipstr in self.ignored_ip:
					loc = _locator.lookup(ipstr)
					if loc:
						loc = ''.join(loc).replace('CZ88.NET', '').strip()
					result.append({
						'domain': item['raw_url'],
						'ip': ipstr,
						'loc': loc,
						'ping_success': ping(ipstr).ret_code == 0
						})
			with open('dns{}.txt'.format(spider.timestamp), 'a', encoding='utf-8') as out:
				for r in result:
					out.write('{}\t{}\t{}\t{}\n'.format(r['domain'], r['ip'], r['loc'], r['ping_success']))
		return item
